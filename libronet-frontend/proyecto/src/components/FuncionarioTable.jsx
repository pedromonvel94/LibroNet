            ...
            <tbody>
              {filtered.map((f, idx) => (
                <tr key={f.numeroDocumento} style={{ animationDelay: `${idx * 40}ms` }}>
                  ...
                  <td><Badge variant="blue">{f.tipoDocumento ?? '—'}</Badge></td>
                  <td className="td-name">{f.nombreCompleto}</td>
                  <td className="td-date">{formatDate(f.fechaIngreso)}</td>
                  <td><Badge variant="purple">{f.estadoCivil ?? '—'}</Badge></td>
                  <td><Badge variant="green">{f.formacionAcademica ?? '—'}</Badge></td>
                  <td>
                    <div className="table-actions">
                      <button className="btn-icon" title="Ver detalle"
                        onClick={() => navigate(`/funcionarios/${f.numeroDocumento}`)}>👁</button>
                      <button className="btn-icon" title="Editar"
                        onClick={() => navigate(`/funcionarios/${f.numeroDocumento}/editar`)}>✏️</button>
                      <button className="btn-icon btn-icon--danger" title="Eliminar"
                        onClick={() => onDelete(f.numeroDocumento)}>🗑</button>
                    </div>
                  </td>
                </tr>
              ))}
